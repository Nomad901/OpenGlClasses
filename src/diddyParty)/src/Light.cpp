...
void PointLight::sendToShader(Shader& pShader)
{
	...
	pShader.setUnifrom1f("point_light.intensity", getIntensity());
	pShader.setUnifrom1f("point_light.constant", mConstant);
	pShader.setUnifrom1f("point_light.linear", mLinear);
	pShader.setUnifrom1f("point_light.quadratic", mQuadratic);
}
...
void PointLight::sendToShaderArray(Shader& pShader, int32_t pIndex)
{
	...
	pShader.setUnifrom1f((point_lightArr  + ".intensity"), getIntensity());
	pShader.setUnifrom1f((point_lightArr  + ".constant"), mConstant);
	pShader.setUnifrom1f((point_lightArr  + ".linear"), mLinear);
	pShader.setUnifrom1f((point_lightArr  + ".quadratic"), mQuadratic);
}

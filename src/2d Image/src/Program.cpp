...
Program::Program(typeMesh pTypeMesh, int32_t pScreenWidht, int32_t pScreenHeight)
{
	...
	case Program::typeMesh::QUAD:
		mMesh3D.initQuad();
		break;
	case Program::typeMesh::QUAD_IMG:
		mMesh3D.initQuadImg();
		break;
	case Program::typeMesh::CUBE:
		mMesh3D.initCube(); 
		break;
	case Program::typeMesh::CUBE_IMG:
		mMesh3D.initCubeImg();
		break;
	default:
		break;
	...
	mTexture.init(resourcePath + "myFriend.png");
}
...
void Program::preDraw()
{
	...
	GLuint tex0Uni = glGetUniformLocation(mGraphicsPipelineProgram, "tex0");

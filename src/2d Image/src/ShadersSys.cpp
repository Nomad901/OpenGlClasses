...
std::string ShadersSys::getStringFromFile(const std::filesystem::path& pPath)
{
	...
	LOG(type::ERROR, "Be it file cant be opened or the file doesnt exist (?)");
	return "";
}
...
GLuint ShadersSys::compileShaders(GLuint pType, std::string_view pShaderSource)
{
	...
	{
		LOG(type::CRITICAL_ERROR, "There is no such a type!");
		std::terminate();
	}
	...
	if (result == GL_FALSE)
	{
		...
		if (pType == GL_VERTEX_SHADER)
			LOG(type::CRITICAL_ERROR, "Couldnt compile the vertex shader!");
		else if (pType == GL_FRAGMENT_SHADER)
			LOG(type::CRITICAL_ERROR, "Couldnt compile the fragment shader!\n");
